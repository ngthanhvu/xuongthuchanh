    ...
    <form action="{{ route('teacher.lessons.store') }}" method="POST">
        ...
        <div class="form-group mb-3">
            ...
            <select name="type" id="type" class="form-control" >
                ...
                <option value="quiz" {{ old('type') == 'quiz' ? 'selected' : '' }}>Bài kiểm tra</option>
                <option value="file" {{ old('type') == 'file' ? 'selected' : '' }}>Tệp</option>
            </select>
            @error('type')
                <span class="text-danger">{{ $message }}</span>
            @enderror
        ...
        <div class="form-group">
            ...
            <a href="{{ route('teacher.lessons.index') }}" class="btn btn-secondary">Hủy</a>
        </div>
    </form>
</div>
@endsection